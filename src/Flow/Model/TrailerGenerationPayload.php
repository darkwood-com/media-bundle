<?php

declare(strict_types=1);

namespace App\Flow\Model;

use App\Application\Trailer\DTO\TrailerDefinition;
use App\Application\Trailer\DTO\TrailerGenerationResult;
use App\Domain\Trailer\TrailerProject;
use App\Infrastructure\Trailer\Rendering\ScenarioConcatResult;
use App\Infrastructure\Trailer\Rendering\SceneClipRenderReport;

/**
 * Mutable Ip payload for trailer generation flows. Mutated in place through prepare,
 * scene processing, and finalize steps (same pattern as GenerateDraftPayload in Flow examples).
 */
final class TrailerGenerationPayload
{
    /** @var list<SceneClipRenderReport> */
    public array $sceneClipReports = [];

    public bool $anyFailed = false;

    /** @var array{json: string, markdown: string}|null */
    public ?array $benchmarkReportPaths = null;

    public ?ScenarioConcatResult $scenarioConcat = null;

    public ?string $renderOutputPath = null;

    public ?TrailerGenerationResult $result = null;

    public function __construct(
        public string $yamlPath,
        /** @var array<string, mixed>|null */
        public ?array $firstSceneVideoOptions = null,
        public ?TrailerDefinition $definition = null,
        public ?TrailerProject $project = null,
        public string $projectId = '',
    ) {
    }

    public function getResult(): TrailerGenerationResult
    {
        if ($this->result === null) {
            throw new \LogicException('Trailer generation did not produce a result (finalize step missing or failed).');
        }

        return $this->result;
    }
}
