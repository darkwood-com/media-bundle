...
final class ScenarioConcatFfmpegRenderer
{
    ...
    public function concatIfPossible(string $projectId, TrailerProject $project): ScenarioConcatResult
    {
        try {
    ...
    private function concatIfPossibleInner(string $projectId, TrailerProject $project): ScenarioConcatResult
    {
        $clipPaths = [];
        ...
        foreach ($scenes as $scene) {
            ...
            if ($this->isValidSceneClip($path)) {
                ...
                $this->logger->info('Trailer scenario concat: scene excluded from scenario (scene.mp4 not usable)', [
                    'project_id' => $projectId,
                    'scene_id' => $scene->id(),
                    'scene_number' => $scene->number(),
                    'reason' => $reason,
        ...
        try {
            ...
            foreach ($stagedIncluded as $row) {
                $this->logger->info('Trailer scenario concat: scene included in scenario.mp4', [
                    'project_id' => $projectId,
